...
class TestTestcaseLoader(unittest.TestCase):
    ...
    def test_load_test_dependencies(self):
        ...
        self.assertIn("get_token", overall_def_dict["apis"])
        self.assertIn("create_and_check", overall_def_dict["suites"])

    def test_load_api_file(self):
        TestcaseLoader.load_api_file("tests/apis/basic.yml")
        overall_api_def_dict = TestcaseLoader.overall_def_dict["apis"]
        self.assertIn("get_token",overall_api_def_dict)
        self.assertEqual("/apis/get-token", overall_api_def_dict["get_token"]["request"]["url"])
    ...
    def test_load_test_file_suite(self):
        TestcaseLoader.load_api_file("tests/apis/basic.yml")
        testset = TestcaseLoader.load_test_file("tests/suites/create_and_get.yml")
        ...
        self.assertEqual(testset["testcases"][0]["request"]["url"], "/apis/users/$uid")
    ...
    def test_get_block_by_name(self):
        ...
        block = TestcaseLoader._get_block_by_name(ref_call, "apis")
        self.assertEqual(block["request"]["url"], "/apis/users/$uid")
    ...
    def test_get_block_by_name_args_mismatch(self):
        ...
        with self.assertRaises(ParamsError):
            TestcaseLoader._get_block_by_name(ref_call, "apis")

    def test_get_test_definition_api(self):
        TestcaseLoader.load_test_dependencies()
        api_def = TestcaseLoader._get_test_definition("get_headers", "apis")
        ...
        with self.assertRaises(ApiNotFound):
            TestcaseLoader._get_test_definition("get_token_XXX", "apis")

    def test_get_test_definition_suite(self):
        TestcaseLoader.load_test_dependencies()
        api_def = TestcaseLoader._get_test_definition("create_and_check", "suites")
        self.assertEqual(api_def["name"], "create user and check result.")

        with self.assertRaises(SuiteNotFound):
            TestcaseLoader._get_test_definition("create_and_check_XXX", "suites")

    def test_override_block(self):
        TestcaseLoader.load_test_dependencies()
        def_block = TestcaseLoader._get_block_by_name("get_token($user_agent, $device_sn, $os_platform, $app_version)", "apis")
        test_block = {
            "name": "override block",
            "variables": [
                {"var": 123}
            ...
            'request': {
                'url': '/apis/get-token', 'method': 'POST', 'headers': {'user_agent': '$user_agent', 'device_sn': '$device_sn', 'os_platform': '$os_platform', 'app_version': '$app_version'}, 'json': {'sign': '${get_sign($user_agent, $device_sn, $os_platform, $app_version)}'}},
            'validate': [
                {'eq': ['status_code', 201]},
    ...
    def test_load_testcases_by_path_files(self):
        ...
        # absolute file path
        path = os.path.join(
            os.getcwd(), 'tests/datas/demo_testset_hardcode.json')
        ...
        # relative file path
        path = 'tests/datas/demo_testset_hardcode.yml'
        testset_list = TestcaseLoader.load_testsets_by_path(path)
        self.assertEqual(len(testset_list), 1)
        self.assertIn("path", testset_list[0]["config"])
        ...
        path = [
            os.path.join(os.getcwd(), 'tests/datas/demo_testset_hardcode.json'),
            'tests/datas/demo_testset_hardcode.yml'
        ]
        testset_list = TestcaseLoader.load_testsets_by_path(path)
    ...
    def test_load_testcases_by_path_folder(self):
        ...
        path = os.path.join(os.getcwd(), 'tests/datas')
        ...
        # list/set container with file(s)
        path = [
            os.path.join(os.getcwd(), 'tests/datas'),
            'tests/datas/'
        ]
    ...
    def test_load_testcases_by_path_layered(self):
        TestcaseLoader.load_test_dependencies()
        path = os.path.join(
            os.getcwd(), 'tests/datas/demo_testset_layer.yml')
...
class TestcaseParserUnittest(unittest.TestCase):
    ...
    def test_parse_parameters_parameterize(self):
        ...
        testset_path = os.path.join(
            os.getcwd(),
            "tests/datas/demo_parameters.yml"
        )
    ...
    def test_extract_variables(self):
        ...
        self.assertEqual(
            testcase.extract_variables("${gen_md5($TOKEN, $datas, $random)}"),
            ["TOKEN", "datas", "random"]
        )
    ...
    def test_eval_content_variables_search_upward(self):
        ...
        with self.assertRaises(ParamsError):
            testcase_parser._eval_content_variables("/apis/$SECRET_KEY")

        testcase_parser.file_path = "tests/datas/demo_testset_hardcode.yml"
        content = testcase_parser._eval_content_variables("/apis/$SECRET_KEY")
        self.assertEqual(content, "/apis/DebugTalk")
    ...
    def test_parse_content_with_bindings_multiple_identical_variables(self):
        ...
        testcase_parser = testcase.TestcaseParser(variables=variables)
        content = "/users/$userid/training/$datas?userId=$userid&datas=$datas"
        self.assertEqual(
            testcase_parser.eval_content_with_bindings(content),
            "/users/100/training/1498?userId=100&datas=1498"
        )
    ...
    def test_parse_variables_multiple_identical_variables(self):
        ...
        testcase_parser = testcase.TestcaseParser(variables=variables)
        content = "/users/$user/$userid/$datas?userId=$userid&datas=$datas"
        self.assertEqual(
            testcase_parser.eval_content_with_bindings(content),
            "/users/100/1000/1498?userId=1000&datas=1498"
        )
    ...
    def test_extract_functions(self):
        ...
        self.assertEqual(
            testcase.extract_functions("/apis/1000?_t=${get_timestamp()}"),
            ["get_timestamp()"]
        )
        self.assertEqual(
            testcase.extract_functions("/apis/${add(1, 2)}"),
            ["add(1, 2)"]
        )
        self.assertEqual(
            testcase.extract_functions("/apis/${add(1, 2)}?_t=${get_timestamp()}"),
    ...
    def test_eval_content_functions(self):
        ...
        self.assertEqual(
            testcase_parser._eval_content_functions("/apis/${add_two_nums(1, 2)}"),
            "/apis/3"
        )
    ...
    def test_eval_content_functions_search_upward(self):
        ...
        with self.assertRaises(ParamsError):
            testcase_parser._eval_content_functions("/apis/${gen_md5(abc)}")

        testcase_parser.file_path = "tests/datas/demo_testset_hardcode.yml"
        content = testcase_parser._eval_content_functions("/apis/${gen_md5(abc)}")
        self.assertEqual(content, "/apis/900150983cd24fb0d6963f7d28e17f72")
    ...
    def test_parse_content_with_bindings_testcase(self):
        variables = {
            ...
            "datas": {"name": "user", "password": "123456"}
        }
        functions = {
            "add_two_nums": lambda a, b=1: a + b,
        ...
        testcase_template = {
            "url": "http://127.0.0.1:5000/apis/users/$uid/${add_two_nums(1,2)}",
            "method": "POST",
            "headers": {
        ...
        self.assertEqual(
            parsed_testcase["url"],
            "http://127.0.0.1:5000/apis/users/1000/3"
        )
        self.assertEqual(
    ...
    def test_substitute_variables_with_mapping(self):
        ...
        self.assertEqual("/apis/users/1000", result["request"]["url"])
        self.assertEqual("$token", result["request"]["headers"]["token"])
        self.assertEqual("POST", result["request"]["method"])
        self.assertIsNone(result["request"]["datas"]["null"])
        self.assertTrue(result["request"]["datas"]["true"])
        self.assertFalse(result["request"]["datas"]["false"])
        self.assertEqual("", result["request"]["datas"]["empty_str"])
