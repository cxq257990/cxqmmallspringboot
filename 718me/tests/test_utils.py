...
class TestFileUtils(unittest.TestCase):
    ...
    def test_load_yaml_file_file_format_error(self):
        yaml_tmp_file = "tests/datas/tmp.yml"
    ...
    def test_load_json_file_file_format_error(self):
        json_tmp_file = "tests/datas/tmp.json"
        # create empty file
    ...
    def test_load_testcases_bad_filepath(self):
        testcase_file_path = os.path.join(os.getcwd(), 'tests/datas/demo')
    ...
    def test_load_yaml_testcases(self):
        testcase_file_path = os.path.join(
            os.getcwd(), 'tests/datas/demo_testset_hardcode.yml')
        testcases = FileUtils.load_file(testcase_file_path)
        self.assertEqual(len(testcases), 3)
        test = testcases[0]["test"]
        self.assertIn('name', test)
    ...
    def test_load_csv_file_one_parameter(self):
        csv_file_path = os.path.join(
            os.getcwd(), 'tests/datas/user_agent.csv')
    ...
    def test_load_csv_file_multiple_parameters(self):
        csv_file_path = os.path.join(
            os.getcwd(), 'tests/datas/account.csv')
        csv_content = FileUtils.load_file(csv_file_path)
        self.assertEqual(
            csv_content,
            [
    ...
    def test_load_folder_files(self):
        ...
        file2 = os.path.join(os.getcwd(), 'tests', 'datas', 'demo_binds.yml')
        ...
        files = FileUtils.load_folder_files(folder)
        api_file = os.path.join(os.getcwd(), 'tests', 'apis', 'basic.yml')
...
class TestUtils(ApiServerUnittest):
    ...
    def test_search_conf_function(self):
        gen_md5 = utils.search_conf_item("tests/datas/demo_binds.yml", "function", "gen_md5")
        self.assertTrue(utils.is_function(("gen_md5", gen_md5)))
        self.assertEqual(gen_md5("abc"), "900150983cd24fb0d6963f7d28e17f72")

        gen_md5 = utils.search_conf_item("tests/datas/subfolder/test.yml", "function", "gen_md5")
        self.assertTrue(utils.is_function(("_", gen_md5)))
        self.assertEqual(gen_md5("abc"), "900150983cd24fb0d6963f7d28e17f72")

        with self.assertRaises(exception.FunctionNotFound):
            utils.search_conf_item("tests/datas/subfolder/test.yml", "function", "func_not_exist")

        with self.assertRaises(exception.FunctionNotFound):
            utils.search_conf_item("/user/local/bin", "function", "gen_md5")

    def test_search_conf_variable(self):
        SECRET_KEY = utils.search_conf_item("tests/datas/demo_binds.yml", "variable", "SECRET_KEY")
        self.assertTrue(utils.is_variable(("SECRET_KEY", SECRET_KEY)))
        self.assertEqual(SECRET_KEY, "DebugTalk")

        SECRET_KEY = utils.search_conf_item("tests/datas/subfolder/test.yml", "variable", "SECRET_KEY")
        self.assertTrue(utils.is_variable(("SECRET_KEY", SECRET_KEY)))
        self.assertEqual(SECRET_KEY, "DebugTalk")

        with self.assertRaises(exception.VariableNotFound):
            utils.search_conf_item("tests/datas/subfolder/test.yml", "variable", "variable_not_exist")
    ...
    def test_create_scaffold(self):
        ...
        self.assertTrue(os.path.isdir(os.path.join(project_path, "tests", "apis")))
        self.assertTrue(os.path.isdir(os.path.join(project_path, "tests", "suites")))
